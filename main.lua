...
-- No Net child indexes to update when the game patches
-----------------------------------------------------------
...
-- Fallback: name search on Net (still no hard indexes)
...
-- Suppress trade UI / notifs (Chocola-style) when controllers exist
...
-- Trade active via ReplicatorClient when available
...
local function startFullAutomation()
	...
	-- GUI fallback for trade open/close
	task.spawn(function()
		...
		pg.DescendantRemoving:Connect(function(obj)
			...
			if n:find("trade") or n:find("brainrottrader") or n:find("tradelive") then
				if currentlyInTrade then
					...
					print("[Brainrot] Trade closed → will re-add next open")
				end
			end
		end)
	end)

	if addBrainrotRemote and #brainrotQueue > 0 then
		task.spawn(function()
			while true do
				forceAddAllBrainrots()
				task.wait(0.35)
	...
	-- Ready + Accept (smarter: only when in trade, re-fire if not ready/accepted)
	task.spawn(function()
	...
	print("[Brainrot] Automation started (getupvalue remotes)")
end
