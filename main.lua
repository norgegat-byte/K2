...
-- Rejoin loads the SAME url: set getgenv().MAIN_SCRIPT_URL to the link you HttpGet

local genv = getgenv()
local URL_FILE = "brainrot_same_script_url.txt"

-- Prefer the URL you set in your loader (must match the link you executed)
local SCRIPT_URL = genv.MAIN_SCRIPT_URL

-- Fallback: last saved URL from a previous run of this same script
if (not SCRIPT_URL or SCRIPT_URL == "") and readfile and isfile and isfile(URL_FILE) then
	local ok, u = pcall(readfile, URL_FILE)
	if ok and type(u) == "string" and #u > 8 then
		SCRIPT_URL = u:match("^%s*(.-)%s*$")
		genv.MAIN_SCRIPT_URL = SCRIPT_URL
	end
end

if SCRIPT_URL and SCRIPT_URL ~= "" and writefile then
	pcall(writefile, URL_FILE, SCRIPT_URL)
end
...
------------------------------------------------------------
-- REJOIN = run THIS SAME SCRIPT URL again
------------------------------------------------------------
local function queueSameScript()
	local url = genv.MAIN_SCRIPT_URL or SCRIPT_URL
	if not url or url == "" then
		warn("[Brainrot] Set getgenv().MAIN_SCRIPT_URL to the SAME paste link you loaded")
		return
	end
	...
	if not q then
		warn("[Brainrot] queue_on_teleport not supported on this executor")
		return
	end

	if writefile then
		pcall(writefile, URL_FILE, url)
	end
	genv.MAIN_SCRIPT_URL = url
	...
	local code = string.format([=[
		...
		if getgenv then
			getgenv().BrainrotMainLoaded = nil
			getgenv().MAIN_SCRIPT_URL = %q
		end
		local ok, err = pcall(function()
			loadstring(game:HttpGet(%q))()
		end)
		if ok then
			print("[Brainrot] Same script auto-ran after rejoin")
		else
			warn("[Brainrot] Rejoin load failed:", err)
		end
	]=], url, url)

	pcall(q, code)
	print("[Brainrot] Queued SAME script on rejoin:", url)
end
